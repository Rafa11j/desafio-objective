const initGame = () => {
  localStorage.clear();
  appState.stage = 0;
  appState.title = 'O prato que você pensou é massa?';
  appState.dish1 = 'Pizza';
  appState.dish2 = 'Bolo de Chocolate';
  appState.dishQuestion1 = '';
  appState.dishReason1 = '';
  appState.dishQuestion2 = '';
  appState.dishReason2 = '';
...
const isPasta = () => {
  console.log('Is Pasta: ', appState.stage);
  if (appState.dishReason1 !== '' && appState.stage === 1) {
    appState.title = appState.dish1 !== 'Pizza' ? 
    `O prato que você pensou é ${appState.dishReason1}?` : 'O prato que você pensou é Pizza?';
    appState.stage = 5;
  } else {
...
const isNotDish = () => {
  if (appState.stage === 10 || appState.stage === 11) {
    appState.stage = 11;
  } else {
    appState.stage = 4;
  }
